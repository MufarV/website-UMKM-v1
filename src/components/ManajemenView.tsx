...
const PlanningView = ({ planning, projects, onUpdate }: { planning: any, projects: any[], onUpdate: (data: any) => void }) => {
  ...
  const removeTarget = (type: 'financial' | 'nonFinancial', id: string) => {
    if (type === 'financial') {
      setData({ ...data, financialTargets: data.financialTargets.filter((t: any) => t.id !== id) });
    } else {
      setData({ ...data, nonFinancialTargets: data.nonFinancialTargets.filter((t: any) => t.id !== id) });
    }
  };
  ...
  const removeSchedule = (id: string) => {
    setData({ ...data, schedules: data.schedules.filter((s: any) => s.id !== id) });
  };
          ...
          {projects.map(proj => (
            <div key={proj.id} className="min-w-[280px] bg-slate-50 border border-slate-200 p-5 rounded-3xl space-y-3">
              <div className="flex justify-between items-center">
                ...
                <button onClick={() => firebaseService.delete('projects', proj.id)} className="text-slate-300 hover:text-rose-500">
        ...
        <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button 
              ...
              className="flex items-center gap-2 px-4 py-1.5 bg-slate-900 text-white rounded-xl text-[10px] font-black uppercase tracking-tight hover:bg-slate-800 transition-all border border-slate-800"
            >
              <Briefcase className="w-3 h-3" /> + Proyek
            </button>
            <button onClick={addSchedule} className="p-1.5 bg-indigo-50 text-indigo-600 rounded-lg hover:bg-indigo-100">
              <Plus className="w-4 h-4" />
            </button>
          </div>
        </h3>
        <div className="space-y-4">
          <div className="grid grid-cols-[1fr_150px_150px_130px_130px_100px_40px] gap-4 px-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">
...
const OrganizingView = ({ divisions, employees, tasks, projects }: { divisions: any[], employees: any[], tasks: any[], projects: any[] }) => {
    ...
    <div className="space-y-12 animate-in slide-in-from-bottom-4 duration-500">
      ...
      <section className="space-y-6 pt-12 border-t border-slate-100">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-xl font-black text-slate-900">Penugasan Tugas Berdasarkan SDM</h2>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {employees.map(emp => {
            const empTasks = tasks.filter(t => t.assigneeId === emp.id);
            return (
              <div key={emp.id} className="bg-white p-6 rounded-[2rem] border border-slate-200 shadow-sm space-y-4">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-slate-50 rounded-xl flex items-center justify-center font-black text-indigo-600">
                    {emp.name[0]}
                  </div>
                  <div>
                    <h4 className="font-black text-slate-800 text-sm leading-none mb-1">{emp.name}</h4>
                    <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{emp.role}</p>
                  </div>
                </div>
                <div className="space-y-2">
                  <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Tugas yang Diemban ({empTasks.length})</p>
                  <div className="space-y-1.5">
                    {empTasks.map(t => (
                      <div key={t.id} className="p-2 bg-slate-50 border border-slate-100 rounded-xl flex justify-between items-center">
                        <span className="text-[10px] font-bold text-slate-600 truncate flex-1">{t.title}</span>
                        <span className={cn(
                          "text-[8px] font-black px-1.5 py-0.5 rounded",
                          t.status === 'Done' ? "bg-emerald-50 text-emerald-600" : "bg-indigo-50 text-indigo-600"
                        )}>{t.status}</span>
                      </div>
                    ))}
                    {empTasks.length === 0 && <p className="text-[9px] text-slate-300 italic font-bold">Belum ada tugas</p>}
                  </div>
                </div>
              </div>
            )
...
const ActuatingView = ({ tasks, employees }: { tasks: any[], employees: any[] }) => {
  const [showAddTask, setShowAddTask] = useState(false);
  const [newTask, setNewTask] = useState({ title: '', priority: 'Medium', status: 'Todo', deadline: '', assigneeId: '', projectId: '' });

  const handleAddTask = async () => {
    if (!newTask.title) return;
    await firebaseService.create('tasks', { ...newTask, ownerId: auth.currentUser?.uid });
    setNewTask({ title: '', priority: 'Medium', status: 'Todo', deadline: '', assigneeId: '', projectId: '' });
    ...
    <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-black text-slate-900">Pengerahan SDM & Delegasi (Actuating)</h2>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Kirim pesan tugas ke tim dan monitor penggerakan</p>
          </div>
        </div>
        <button 
          onClick={() => setShowAddTask(!showAddTask)}
          className="flex items-center gap-2 px-6 py-3 bg-slate-900 text-white rounded-2xl font-black shadow-lg hover:bg-slate-800 transition-all text-xs"
        >
      ...
      <AnimatePresence>
        {showAddTask && (
          <motion.div 
             ...
             animate={{ opacity: 1, scale: 1 }}
             exit={{ opacity: 0, scale: 0.95 }}
             className="bg-indigo-900 p-8 rounded-[2.5rem] text-white space-y-6 shadow-2xl"
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <input 
                placeholder="Apa tugasnya?" 
                className="w-full px-5 py-3 bg-white/10 border border-white/20 rounded-2xl outline-none font-bold placeholder:text-white/30"
                value={newTask.title}
                onChange={e => setNewTask({...newTask, title: e.target.value})}
              />
              <select 
                className="w-full px-5 py-3 bg-white/10 border border-white/20 rounded-2xl outline-none font-bold"
                value={newTask.assigneeId}
                onChange={e => setNewTask({...newTask, assigneeId: e.target.value})}
              >
                <option value="" className="text-slate-900">Pilih Penanggung Jawab...</option>
                {employees.map(emp => <option key={emp.id} value={emp.id} className="text-slate-900">{emp.name}</option>)}
              </select>
            </div>
            <div className="flex justify-end gap-3">
              <button onClick={() => setShowAddTask(false)} className="px-6 py-2 text-white/50 font-black text-xs uppercase">Batal</button>
              <button onClick={handleAddTask} className="bg-white text-indigo-900 px-8 py-2 rounded-xl font-black text-xs uppercase">Simpan & Siapkan Delegasi</button>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="bg-white rounded-[2.5rem] border border-slate-200 overflow-hidden shadow-sm">
        <div className="overflow-x-auto">
                ...
                return (
                  <tr key={task.id} className="hover:bg-slate-50/50 transition-colors">
                    <td className="p-5">
                      ...
                      <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Deadline: {task.deadline || '-'}</p>
                    </td>
                    <td className="p-5">
                      <div className="flex items-center gap-2">
                    ...
                    <td className="p-5">
                      <span className="px-2 py-1 bg-indigo-50 text-indigo-600 rounded-lg text-[9px] font-black uppercase tracking-widest">{task.status}</span>
                    </td>
                    <td className="p-5 text-center">
                      <div className="flex items-center justify-center gap-2">
                        ...
                        <button className="p-2 border border-slate-200 rounded-xl hover:bg-white text-slate-400 hover:text-indigo-600 transition-colors">
          ...
          {tasks.filter(t => t.status !== 'Done').length === 0 && (
            <div className="py-20 text-center text-slate-300 font-bold text-xs uppercase italic">Tidak ada tugas aktif untuk didelegasikan</div>
          )}
        </div>
      </div>
    </div>
...
const ControllingView = ({ tasks, projects, planning, employees }: { tasks: any[], projects: any[], planning: any, employees: any[] }) => {
  ...
  return (
    <div className="space-y-12 animate-in slide-in-from-bottom-4 duration-500 pb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-white p-6 rounded-[2rem] border border-slate-200 shadow-sm space-y-2">
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none">Proyek Aktif</p>
      ...
      {/* SDM Performance Matrix */}
      <section className="space-y-6">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-slate-900 rounded-xl flex items-center justify-center text-white">
            <TrendingUp className="w-5 h-5" />
          </div>
          <h2 className="text-xl font-black text-slate-900">Evaluasi Kinerja SDM (Insight, Skill, & Motivasi)</h2>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {employees.map(emp => (
            <div key={emp.id} className="bg-white p-8 rounded-[2.5rem] border-2 border-slate-50 shadow-sm space-y-6 transition-all hover:border-indigo-100 hover:shadow-xl group">
              <div className="flex items-center gap-4">
                <div className="w-14 h-14 bg-indigo-50 border border-indigo-100 rounded-2xl flex items-center justify-center text-indigo-600 font-black text-xl">
                  {emp.name[0]}
                </div>
                <div>
                  <h4 className="font-black text-slate-800 text-lg leading-tight">{emp.name}</h4>
                  <p className="text-[10px] font-bold text-indigo-600 uppercase tracking-widest">{emp.role}</p>
                </div>
              </div>

              <div className="space-y-5">
                {[
                  { label: 'Insight (Wawasan)', val: emp.insight || 0, color: 'indigo' },
                  { label: 'Skill (Keahlian)', val: emp.skill || 0, color: 'emerald' },
                  { label: 'Motivation (Daya Juang)', val: emp.motivation || 0, color: 'amber' }
                ].map(item => (
                  <div key={item.label} className="space-y-1.5">
                    <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-tighter">
                      <span className="text-slate-400">{item.label}</span>
                      <span className={`text-${item.color}-600`}>{item.val}%</span>
                    </div>
              ...
              <div className="pt-2 border-t border-slate-50">
                <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest leading-relaxed">
                  Catatan Review: Staf menunjukkan {Math.max((emp.insight || 0), (emp.skill || 0), (emp.motivation || 0)) > 80 ? 'performa luar biasa' : 'progres yang stabil'} dalam periode ini.
                </p>
              </div>
            </div>
          ))}
...
export const ManajemenView = () => {
  ...
  useEffect(() => {
    ...
    const unsubTasks = firebaseService.subscribe('tasks', setTasks);
  ...
  const columns = [
    { id: 'Todo', title: 'Belum Mulai', icon: Clock, color: 'text-slate-700', bg: 'bg-slate-200' },
    { id: 'In Progress', title: 'Sedang Berjalan', icon: Layers, color: 'text-indigo-800', bg: 'bg-indigo-600' },
    { id: 'Done', title: 'Selesai', icon: CheckCircle2, color: 'text-emerald-800', bg: 'bg-emerald-600' },
  ];
  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight text-slate-900 mb-2">Manajemen Operasional 💼</h1>
          <p className="text-slate-600 font-medium leading-relaxed">Kelola tim, delegasikan tugas, dan pantau produktivitas UMKM Anda.</p>
        </div>
        <div className="flex gap-3">
          <div className="bg-white px-6 py-3 rounded-2xl border-2 border-slate-100 flex items-center gap-4 shadow-sm">
            <div className="text-right">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Total Tim</p>
              <p className="text-xl font-black text-slate-900">{employees.length}</p>
            </div>
            <div className="w-px h-8 bg-slate-100"></div>
            <div className="text-right">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Tugas Aktif</p>
              <p className="text-xl font-black text-indigo-600">{tasks.filter(t => t.status !== 'Done').length}</p>
            </div>
          </div>
        </div>
      </div>
